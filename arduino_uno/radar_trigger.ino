...
const int radarServoPin = 12;  // Servo chính cho radar
...
void loop() {
  ...
  for (int i = 15; i <= 165; i++) {  
    ...
    radarServo.write(i);
    delay(30); // Đợi radar servo di chuyển
    ...
    if (filteredDistance > 0 && filteredDistance < detectionThreshold && !objectDetected) {
      if (confirmObject()) {
        ...
        delay(100);  // Đợi laser ổn định
        
        // Gửi tín hiệu đến ESP32-CAM để chụp ảnh
        triggerCamera();
        
        // Giữ laser bật trong 2 giây
        delay(2000);
